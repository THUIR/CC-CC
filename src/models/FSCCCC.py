...
class FSCCCC(CCCC):
    ...
    def _init_graph(self):
        with self.graph.as_default():
            ...
            f_bias = tf.nn.embedding_lookup(self.weights['cross_bias'], self.train_features[:, 2:])
            f_bias = f_bias * (1 - drop_f_pos)
            # self.check_list.append(('drop_f_pos', drop_f_pos))
            # self.check_list.append(('f_bias', f_bias))
            self.cross_bias = tf.reduce_sum(tf.reduce_sum(f_bias, axis=1), axis=1)
            ...
            # cb part
            # train_features = self.train_features * tf.cast(1 - self.drop_pos, dtype=tf.int32)
            # u_fs = train_features[:, 2:2 + self.user_feature_num]
            # i_fs = train_features[:, 2 + self.user_feature_num:]
            ...
            # self.prediction = tf.sigmoid(self.prediction) * 4 + 1
